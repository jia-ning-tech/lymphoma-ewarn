...
def load_preds(h: int, split: str, calibrated: str|None) -> pd.DataFrame:
    if calibrated:
        ...
        df = pd.read_parquet(p)
        # 优先 prob_cal → prob；若存在原始 prob 先删
...
def main():
    ...
    # 载入并合并
    raw   = load_raw(args.horizon)     # 一定含 'label'
    preds = load_preds(args.horizon, args.split, args.calibrated)  # 一定含 'prob'
    ...
    # 合理的 y 轴范围；如你更希望自适应，可去掉 ylim 参数
